...
def write_complete_verilog(multiplier, n_bits, Z):
    ...
    topflag = True 
    code = ""      
    code = oc.generate_config(config=mult_dict, code=code, key=(0, 16, 16, 'rr'), topflag=True, name="rr_16x16")
...
def get_values(data, Z):
    ...
    subprocess.run(["python3", "yosys_synth_automate.py", f"{n_bits}", str(Z), "rr_16x16"], check=True, 
                #    stdout=subprocess.DEVNULL, 
                #    stderr=subprocess.DEVNULL
                   )
    subprocess.run(["python3", "yosys_area_automate.py",  f"{n_bits}", str(Z), "rr_16x16"], 
                   check=True, stdout=subprocess.DEVNULL, 
                   stderr=subprocess.DEVNULL
                   )
    subprocess.run(["python3", "sta_delay_power_automate.py",  f"{n_bits}", str(Z), "rr_16x16"], check=True, 
                   stdout=subprocess.DEVNULL, 
                   stderr=subprocess.DEVNULL
                   )
